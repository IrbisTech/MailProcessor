	...
	public class Pop3Client : Disposable
	{
		...
		public void Connect(string hostname, int port, bool useSsl, int receiveTimeout, int sendTimeout, RemoteCertificateValidationCallback certificateValidator)
		{
            ...
            if (useSsl)
            {
                ...
                SslStream sslstream = new SslStream(clientSocket.GetStream(), false, new RemoteCertificateValidationCallback(ValidateServerCertificate), null);
                ServicePointManager.ServerCertificateValidationCallback = delegate { return true; };
                certificateValidator = new RemoteCertificateValidationCallback(ValidateServerCertificate);

                //NetworkCredential myCred = new NetworkCredential("hyphenvm\\svc_opensky", "p3n5ky@18", "");
        ...
        public void Connect(string hostname, int port, bool useSsl, int receiveTimeout, int sendTimeout, RemoteCertificateValidationCallback certificateValidator, String Username, String Password)
        {
            AssertDisposed();
            ...
            //const string server = "securemail.hollard.co.za";
            //const int port = 25;
            using (var client = new TcpClient(hostname, port))
            {
                ...
                using (var sslStream = new SslStream(stream))
                {
                    ...
                    clearTextWriter.WriteLine("USER " + Username);
                    clearTextWriter.WriteLine("PASS " + Password);
                    Connected = true;

                    //startTlsResponse = clearTextReader.ReadLine();
        ...
        public Message GetMessageFromFile(string filename)
        {
            byte[] buff = null;
